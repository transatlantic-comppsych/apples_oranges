...
df_overall_baseline_results %>% 
  ...
  set_table_properties(layout = "autofit") %>%  # Autofit column widths
...
As can be seen in @tbl-cdrs-baseline and @tbl-hamd-baseline (see Supplementary Materials), the baseline scores in medication studies were on average substantially higher than psychotherapy studies for the CDRS and the HAM-D. The differences between medication control and psychotherapy control arms were significant at p\<0.05 for the CDRS, though they did not reach statistical significance for the HAM-D (p = `r round(results_hamd_metareg_baseline[[1]][results_hamd_metareg_baseline[[1]]$condition == "psychotherapy_control", "p.value"], 3)`).
...
```
The following tables still need some work including adding n
