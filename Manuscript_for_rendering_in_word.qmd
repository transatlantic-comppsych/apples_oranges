---
title: "Fundamental Problems with Child and Adolecsent Depression Treatments: A Quantitative Reappraisal of the Evidence Base"
author:
...
We then conducted a series of sensitivity analyses of our results. For each of the metanalyses we excluded 1) studies which recruited participants with subclinical levels of depression and 2) studies that used waitlist as their control condition.
...
# This is for overall sample only, with separate sensitivity analyses in the supplement
...
# Too basic, limited formatting options
# 
# knitr:: kable(df_overall_baseline_results, caption="Sample characteristics at baseline across medication and psychotherapy studies")
# 
# gt(df_overall_baseline_results) %>%
#   tab_header(
#     title = md("Test this"),
#     subtitle = md("And this"))
# 
# This doesn't work when rendering to word doc
# 
#   kbl(align = "l", caption = "Sample characteristics at baseline across medication and psychotherapy studies", escape = F) %>%
...
# 
# this doesn't have a caption when rendering to word
# 
# flextable(df_overall_baseline_results) %>%
#   set_caption("Sample characteristics at baseline across medication and psychotherapy studies") %>%
...
df_overall_baseline_results <- df_overall_baseline_results %>% 
  ...
  add_header_row(values = "Table 1: Sample characteristics at baseline across medication and psychotherapy studies", colwidths = 8) %>% 
  set_caption("Table 1: Sample characteristics at baseline across medication and psychotherapy studies") %>%
  bold(part = "header") %>%  # Bold the header row
  bold(i = c(1, 2, 5, 8, 11, 12, 15, 18, 21, 22, 25, 28), j = NULL, bold = TRUE, part = "body") %>%  # Bold specific rows
  border_outer(border = small_border) %>%  # Add outer borders
  hline(part = "all", border = small_border) %>%  # Add light grey horizontal lines all  rows
  hline(part = "header", border = big_border) %>%
  vline(j = 1, border = small_border) %>%
  set_table_properties(width = .75) %>%  # Set table width
  bg(bg = "#F2F2F2", part = "header") %>%  # Add light grey background to header row
  bg(i = c(1, 11, 21), bg  = "#F2F2F2", part = "body") %>% 
  autofit() %>%  # Autofit column widths
  font(fontname = "Calibri", part = "all") %>%   # Set font to Calibri
  padding(i = c(3, 4, 6, 7, 9, 10, 13, 14, 16, 17, 19, 20, 23, 24, 26, 27, 29, 30), j = 1, padding.left = 10) %>%
  align(j = 2:ncol(df_overall_baseline_results), align = "center", part = "all") %>% 
...
colnames(sites_t_test_stats) <- c("Statistic", "df", "p-value", "Lower CI", "Upper CI")
...
flextable(df_sites_summary_stats) %>%
  set_caption ("Table 2: Number of sites across medication and psychotherapy studies") %>%
  bold(part = "header") %>%
  border_outer(border = small_border) %>%
  hline(part = "all", border = small_border) %>%
  hline(part = "header", border = big_border) %>%
  vline(j = 1, border = small_border) %>%
...
flextable(sites_t_test_stats) %>%
  set_caption ("Results for t-test comparing number of sites between medication and psychotherapy studies", style = "Table Caption") %>%
  bold(part = "header") %>%  
  border_outer(border = small_border) %>%  
  hline(part = "header", border = big_border) %>% 
  set_table_properties(width = .75) %>% 
...
# This is for if we want to render to pdf 

# df_sites_summary_stats <- df_sites_summary_stats %>%
#   kbl(caption = "Number of sites across medication and psychotherapy studies") %>%
#   kable_material(lightable_options = c("hover"), full_width = FALSE) %>%
#   row_spec(row = 0, background = "#EBEBEB") %>%
#   column_spec(column = 1, border_right = T)

# sites_t_test_stats %>%
#   kbl(align = "l", caption = "Results for t-test comparing number of sites between medication and psychotherapy studies") %>%
#   kable_material(lightable_options = c("hover"), full_width = FALSE) %>%
#   row_spec(row = 0, background = "#EBEBEB") %>%
#   column_spec(column = 1, border_right = T)
...
There was a significant difference between the number of sites in medication versus psychotherapy trials, as can be seen in Tables 2 and 3. Average number of trial sites was significantly higher across medication studies (*M* = `r df_sites_summary_stats$Mean[df_sites_summary_stats$Modality == "Medication"]`, *SD* =`r df_sites_summary_stats$SD[df_sites_summary_stats$Modality == "Medication"]`) compared to psychotherapy studies (*M* =`r df_sites_summary_stats$Mean[df_sites_summary_stats$Modality == "Psychotherapy"]`, *SD* =`r df_sites_summary_stats$SD[df_sites_summary_stats$Modality == "Psychotherapy"]`)(*t* (`r round (sites_t_test$parameter, 2)`) = `r round(sites_t_test$statistic,2)`, *p* =`r sites_t_test$p.value`). Of those studies for which we had data on number of sites, 26 of 28 (93%) medication trials were multisite, compared to 24 of 45 (54%) psychotherapy studies.
...
flextable(df_mean_coefs_from_sim) %>%
  set_caption ("Summary statistics of the estimated SMDs") %>%
  bold(part = "header") %>%  
  border_outer(border = small_border) %>%  
  hline(part = "header", border = big_border) %>% 
  set_table_properties(width = .75) %>% 
  ...
  autofit() %>% 
  font(fontname = "Calibri", part = "all") %>%   
  align(j = 2:ncol(df_mean_coefs_from_sim), align = "center")

```

![Figure 1.](plot_means_all.png)

![](plot_means_all.pdf)

In Table 5, we present the regression that tests our hypothesis about differences between medication and psychotherapy controls. In particular, in this metaregression model, medication control is the reference category (termed intercept) to which all other categories of the dummy variable are compared. The strongest difference between arms, as judged by the z-value, is between the psychotherapy and medication controls with a z-value `r aggregate_results_overall[aggregate_results_overall$condition == "psychotherapy_control", ]$z_value` , which yields a very low p-value ( `r sprintf("%.3e",(1-(pnorm(8.949921))))` ).

```{r, warning=FALSE, message = FALSE, echo = FALSE}

#aggregate_results_overall

flextable(aggregate_results_overall) %>%
  set_caption ("Metaregression results") %>%
  bold(part = "header") %>%  
  border_outer(border = small_border) %>%  
  hline(part = "header", border = big_border) %>% 
  set_table_properties(width = .75) %>% 
  ...
  autofit() %>% 
  font(fontname = "Calibri", part = "all") %>%   
  align(j = 2:ncol(aggregate_results_overall), align = "center")

```

##### Sensitivity analyses

We then conducted a series of sensitivity analyses of our results.First, we analysed the data after excluding waitlist control studies. As can be seen, the pattern of results is very similar to that of the overall analyses.
...
control_summary <- control_summary %>%
  mutate(Column = case_when(
    ...
    Column == "freq_weeks" ~ "Frequency (weeks)",
    Column == "length_sessions_mins" ~ "Session length (mins)",
    Column == "total_hours" ~ "Total intervention hours",
    Column == "total_period_weeks" ~ "Total intervention period (weeks)",
...
as_grouped_data(control_summary, groups = "Column") %>% 
  as_flextable(hide_grouplabel = TRUE ) %>% 
  set_caption ("Comparing the intensity of the intervention between active and control arms of psychotherapy studies") %>%
  bold(part = "header") %>%
  bold(i = c(1, 4, 7, 10, 13), j = NULL, bold = TRUE, part = "body") %>%  
  border_outer(border = small_border) %>%  
  hline(part = "all", border = small_border) %>%  # Add light grey horizontal lines all  rows
  hline(i = c(3, 6, 9, 12), border = big_border) %>%  # Add darker horizontal lines after specific rows %>% 
  hline(part = "header", border = big_border) %>% 
...
flextable(control_summary_t_tests) %>%
  set_caption ("Results for t-tests comparing intervention intensity between active and control arms of psychotherapy trials", style = "Table Caption") %>%
  bold(part = "header") %>%  
  border_outer(border = small_border) %>%  
  hline(part = "header", border = big_border) %>% 
  set_table_properties(width = .75) %>% 
...
Overall, the active arms of psychotherapy studies were considerably more intensive than the control arms they were compared against (see Table 6 for summary statistics). Active intervention arms featured significantly more sessions than control arms (*t* (`r control_summary_t_tests$df[control_summary_t_tests$Outcome == "Number of sessions"]`) = `r control_summary_t_tests$"t statistic"[control_summary_t_tests$Outcome == "Number of sessions"]`, *p* = `r control_summary_t_tests$"p-value"[control_summary_t_tests$Outcome == "Number of sessions"]`). These sessions were also significantly longer (*t* (`r control_summary_t_tests$df[control_summary_t_tests$Outcome == "Session length (mins)"]`) = `r control_summary_t_tests$"t statistic"[control_summary_t_tests$Outcome == "Session length (mins)"]`, *p* = `r control_summary_t_tests$"p-value"[control_summary_t_tests$Outcome == "Session length (mins)"]`) and more frequent (*t* (`r control_summary_t_tests$df[control_summary_t_tests$Outcome == "Frequency (weeks)"]`) = `r control_summary_t_tests$"t statistic"[control_summary_t_tests$Outcome == "Frequency (weeks)"]`, *p* = `r control_summary_t_tests$"p-value"[control_summary_t_tests$Outcome == "Frequency (weeks)"]`). The total hours involved in an intervention were higher in active vs control arms (*t* (`r control_summary_t_tests$df[control_summary_t_tests$Outcome == "Total intervention hours"]`) = `r control_summary_t_tests$"t statistic"[control_summary_t_tests$Outcome == "Total intervention hours"]`, *p* = `r control_summary_t_tests$"p-value"[control_summary_t_tests$Outcome == "Total intervention hours"]`), though the total period of the intervention was similar (*t* (`r control_summary_t_tests$df[control_summary_t_tests$Outcome == "Total intervention period (weeks)"]`) = `r control_summary_t_tests$"t statistic"[control_summary_t_tests$Outcome == "Total intervention period (weeks)"]`, *p* = `r control_summary_t_tests$"p-value"[control_summary_t_tests$Outcome == "Total intervention period (weeks)"]`).
...
As can be seen in Tables X and Y, the baseline scores in medication studies were on average substantially higher for the CDRS (10.9 points for the control arms) and the HAM-D (7.3 points for the control arms), respectively. The differences were significant at p\<0.05 for CDRS, thought they did not reach statistical significance for the HAM-D (p = 0.0573).
...
We next addressed potential regression to the mean by residualising SMDs by the baseline scores in one regression model.
...
Table X shows the adjusted SMDs with their confidence intervals that we estimated in metaregression. As can be inferred from the non-overlapping confidence intervals, there were significant differences between the medication control and psychotherapy control arms.
...
Similarly, we found that on average there were about 7% more females in psychotherapy trials. Whilst there is little evidence that sex or sex moderate treatment response within modality in adult trials of depression [@cuijperssexPREDICTORMODERATOR2014], this is yet another indicator that different people enter trials of each modality type, which would violate basic assumptions of comparability between trials.
...
PRISMA chart summarising the screening process for the systematic review.
