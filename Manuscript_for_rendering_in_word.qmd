...
# 
# this doesn't have a caption when rendering to word. workaround below, adding caption as header row.
# 
...
df_overall_baseline_results %>% 
  ...
  paginate(group = "category", group_def = "rle") %>% 
  bold(i = c(1, 2, 5, 8, 11, 12, 15, 18, 21, 22, 25, 28), j = NULL, bold = TRUE, part = "body") %>%  # Bold specific rows
  set_table_properties(width = 1, layout = "autofit") %>%  # Set table width
  ...
  bold(i = c(1, 3), part = "header") %>%   # Bold the relevant header rows
  bg(i = 3, bg = "#F2F2F2", part = "header") %>%   # Add light grey background to header row %>% 
  ...
  hline(i = 3, part = "header", border = big_border) %>% 
...
There was a significant difference between the number of sites in medication versus psychotherapy trials, as can be seen in Table 2. Average number of trial sites was significantly higher across medication studies (*M* = `r df_sites_summary_stats$Mean[df_sites_summary_stats$Modality == "Medication"]`, *SD* =`r df_sites_summary_stats$SD[df_sites_summary_stats$Modality == "Medication"]`) compared to psychotherapy studies (*M* =`r df_sites_summary_stats$Mean[df_sites_summary_stats$Modality == "Psychotherapy"]`, *SD* =`r df_sites_summary_stats$SD[df_sites_summary_stats$Modality == "Psychotherapy"]`)(*t* (`r round (sites_t_test$parameter, 2)`) = `r round(sites_t_test$statistic,2)`, *p* =`r sites_t_test_stats$p_value`). Of those studies for which we had data on number of sites, 26 of 28 (93%) medication trials were multisite, compared to 24 of 45 (54%) psychotherapy studies.
...
In Table 4, we present the regression that tests our hypothesis about differences between medication and psychotherapy controls. In particular, in this metaregression model, medication control is the reference category (termed intercept) to which all other categories of the dummy variable are compared. The strongest difference between arms, as judged by the z-value, is between the psychotherapy and medication controls with a z-value `r aggregate_results_overall[aggregate_results_overall$condition == "psychotherapy_control", ]$z_value` , which yields a very low p-value ( `r sprintf("%.3e",(1-(pnorm(8.949921))))` ).
...
Finally, we examined the data including only those studies that used the CDRS, which yielded a very similar pattern of results.
...
Figure S1: PRISMA chart summarising the screening process for the systematic review.

![Figure S1](PRISMA.png)
