...
Psychotherapy trials featured a significantly higher percentage of women when compared to medication trials (`results_met_perc_women_overall$pval_between`). On average, samples were comprised of `r results_met_perc_women_overall$TE_random[results_met_perc_women_overall$subgroup == 1]`% women across psychotherapy trials and `r results_met_perc_women_overall$TE_random[results_met_perc_women_overall$subgroup == 0]`% women across medication trials.  Excluding sub-clinical studies yielded very similar results (p = `results_met_perc_women_clin$pval_between`). This difference in gender composition however did not reach statistical significance when only CBT, fluoxetine and escitalopram trials were included (p = `results_met_perc_women_cbt$pval_between`). 
...
Mean age was `r results_met_age$TE_random[results_met_perc_women_overall$subgroup == 1]` (`r results_met_age$seTE_random[results_met_perc_women_overall$subgroup == 1]`) across psychotherapy trials and `r results_met_age$TE_random[results_met_perc_women_overall$subgroup == 0]` (`r results_met_age$seTE_random[results_met_perc_women_overall$subgroup == 0]`) across medication trials, with no significant between group differences (p = `r results_met_age$pval_between`). There were no significant differences in mean age between modalities on further sensitivity analyses.
...
On average, severity of depressive symptoms at baseline was significantly higher in medication trials when compared to psychotherapy trials, for both the overall sample (p = `results_met_sev$pval_between`) and when taking only those studies where the active arm was eiter CBT, fluoxetine or escitalopram (p = `results_met_sev_cbt$pval_between`). When excluding psychotherapy studies that recruited samples with sub-clinical depression, this difference no longer reached statistical significance (p = `results_met_sev_clin$pval_between`). 
...
# Now lets make a table that combines across these baseline characteristics variables
...
df_overall_baseline_results %>%
  kbl(caption = "Sample characteristics at baseline") %>%
  kable_styling(bootstrap_options = c("hover", "condensed", "bordered"), full_width = FALSE) %>%
  row_spec(row = 0, background = "#D3D3D3",  extra_css = "border-bottom: 2px solid grey;" ) %>% 
  row_spec(c(1, 4, 7), bold = T, extra_css = "border-top: 2px solid grey;") %>%
...
df_sites_summary_stats %>%
  kbl(col.names = c("Modality", "N", "Mean", "SD")) %>% 
  kable_styling(bootstrap_options = c("hover", "condensed", "bordered"), full_width = FALSE) 
