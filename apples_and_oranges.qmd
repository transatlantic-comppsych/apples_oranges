...
We will test in a metaregression for differences between psychotherapy versus medication trials in sample characteristics at baseline. 
...
We also conducted metaregressions to compare psychotherapy vs medication trials on the gender composition of the sample, mean age, and number of trial sites. 
...
Using a random effects meta-regression we compared psychotherapy versus medication trials on four domains: gender composition, age, baseline depression severity levels, and number of trial sites. We first examined each of these across all studies, and then conducted two sensitivity analyses where we excluded psychotherapy studies which 1) recruited participants with sub clinical depression levels and 2) did not use CBT as the active intervention. 

### Gender

Gender composition was similar across psychotherapy and medication trials, 
...
met_perc_women <- metagen(TE.random = percent_women,
                           seTE.random = percent_women_std_error,
...
summary_results <- summary(met_perc_women, byvar = "subgroup")

# Convert the results to a dataframe
result_df <- data.frame(
  TE = summary_met_perc_women$TE.random,
  seTE = summary_met_perc_women$seTE.random,
  lower = summary_met_perc_women$lower.random,
  upper = summary_met_perc_women$upper.random
)
...
knitr:: kable(summary_table, "latex", caption = "Gender composition")
...
#### Quality and nature of psychotherapy controls across trials
