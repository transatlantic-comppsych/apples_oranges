...
# F. Estimate Means and CIs for the overall sample using baseline percentage variable ----------------------------------

# count studies - same as for overall sample
n_unique_studies <- length(unique(df_long_for_metan$new_study_id)) 
df_count_studies <- count_studies(df_long_for_metan )

# specify model
model_1 <- as.formula(~ four_level_var + perc_baseline_mean )

# run metareg function 
# here you can use the simulated results directly
list_model_1_meta_reg_perc <- run_metaregression(list_df_simulated, model_1)
