...
knitr:: kable(results_met_perc_women_overall, caption = "Gender composition")
...
knitr:: kable(results_met_perc_women_clin, caption = "Gender composition in clinical studies")
...
knitr:: kable(results_met_perc_women_cbt, caption = "Gender composition in studies with CBT, escitalopram or fluoxetine")
...
knitr:: kable(results_met_perc_women_no_wl, caption = "Gender composition in studies with CBT, escitalopram or fluoxetine")


```
### Gender

Psychotherapy trials featured a significantly higher percentage of women when compared to medication trials (`results_met_perc_women_overall$pval_between`). On average, samples were comprised of `r results_met_perc_women_overall$TE_random[results_met_perc_women_overall$subgroup == 1]`% (*SE* = `r results_met_perc_women_overall$seTE_random[results_met_perc_women_overall$subgroup == 1]` women across psychotherapy trials and `r results_met_perc_women_overall$TE_random[results_met_perc_women_overall$subgroup == 0]`% (*SE* = `r results_met_perc_women_overall$seTE_random[results_met_perc_women_overall$subgroup == 0]`women across medication trials.  Excluding sub-clinical studies yielded very similar results (p = `results_met_perc_women_clin$pval_between`). This difference in gender composition however did not reach statistical significance when only CBT, fluoxetine and escitalopram trials were included (p = `results_met_perc_women_cbt$pval_between`). 
...
knitr:: kable(results_met_age, caption = "Average age in psychotherapy vs medication trials")
...
knitr:: kable(results_met_age_clin, caption = "Average age in clinical studies")
...
knitr:: kable(results_met_age_cbt, caption = "Average age in cbt, fluox, escit studies")
...
knitr:: kable(results_met_sev, caption = "Baseline Depression Severity in Psychotherapy vs Medication trials")
...
knitr:: kable(results_met_sev_clin, caption = "Baseline Depression Severity in Psychotherapy vs Medication trials, excluding subclinical studies")
...
knitr:: kable(results_met_sev_cbt, caption = "Baseline Depression Severity in Psychotherapy vs Medication trials with CBT, fluoxetine and escitaloptram as the active intervention")

# plot for overall severity results

ggplot(df_for_baseline_severity, aes(x = factor(psy_or_med), y = perc_baseline_mean, color = factor(psy_or_med))) +
  geom_point(position = position_jitter(width = 0.2), size = 2) +
  labs(
    title = "Baseline Depression Severity in Psychotherapy vs Medication trials",
    x = "Treatment Modality",
    y = "Baseline Depression Levels (transformed)",
    color = "Treatment Type"
  ) +
  scale_color_discrete(labels = c("Medication", "Psychotherapy"))
...
df_overall_baseline_results %>%
  kbl(caption = "Table X: Sample characteristics at baseline across medication and psychotherapy studies") %>%
  ...
  footnote(alphabet = c("Values for baseline severity represent the percentage transformed depression scores", "pval_between = the p-value from the test for subgroup differences from the relevant meta-analysis"))
  
```
### Baseline severity

On average, severity of depressive symptoms at baseline was significantly higher in medication trials when compared to psychotherapy trials, for both the overall sample (p = `results_met_sev$pval_between`) and when taking only those studies where the active arm was either CBT, fluoxetine or escitalopram (p = `results_met_sev_cbt$pval_between`). When excluding psychotherapy studies that recruited samples with sub-clinical depression, this difference did not reach statistical significance (p = `results_met_sev_clin$pval_between`). 
