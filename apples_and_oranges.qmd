...
extract_stats <- function(df) {
  ...
  df_stats <- rbind(data_frame(subgroup = NA, k_study = NA, TE_random = NA, seTE_random = NA, 
...
On average, severity of depressive symptoms at baseline was significantly higher in medication trials when compared to psychotherapy trials for the overall sample (p = `results_met_sev$pval_between`).The statistics provided in Table 1 represent the transformed mean scores at baseline on the 

and when taking only those studies where the active arm was either CBT, fluoxetine or escitalopram (p = `results_met_sev_cbt$pval_between`). When excluding psychotherapy studies that recruited samples with sub-clinical depression, this difference did not reach statistical significance (p = `results_met_sev_clin$pval_between`). 
...
Psychotherapy trials featured a significantly higher percentage of women when compared to medication trials (`results_met_perc_women_overall$pval_between`). On average, samples were comprised of `r results_met_perc_women_overall$TE_random[results_met_perc_women_overall$subgroup == 1]`% (*SE* = `r results_met_perc_women_overall$seTE_random[results_met_perc_women_overall$subgroup == 1]` women across psychotherapy trials and `r results_met_perc_women_overall$TE_random[results_met_perc_women_overall$subgroup == 0]`% (*SE* = `r results_met_perc_women_overall$seTE_random[results_met_perc_women_overall$subgroup == 0]`women across medication trials. Excluding sub-clinical studies (p = `results_met_perc_women_clin$pval_between`) and studies that used waitlist as their control condition yielded very similar results (p = `results_met_perc_women_no_wl$pval_between`). This difference in gender composition however did not reach statistical significance when only CBT, fluoxetine and escitalopram trials were included (p = `results_met_perc_women_cbt$pval_between`). 
...
df_overall_baseline_results <- rbind(results_met_perc_women_overall, results_met_age, results_met_sev)

# For row names
df_overall_baseline_results <- df_overall_baseline_results %>%
  mutate(subgroup = case_when(
    row_number() == 1 ~ "Percent Women",
    row_number() == 4 ~ "Age",
    row_number() == 7 ~ "Severity",
...
df_overall_baseline_results %>%
  kbl(align = "l", caption = "Table X: Sample characteristics at baseline across medication and psychotherapy studies", escape = F) %>%
...
sites_t_test_stats <- sites_t_test_stats %>%
  dplyr:: mutate(across(.cols = c(statistic, df, ci_lower, ci_upper), ~ round(., 2)))

sites_t_test_stats %>%
  kbl(caption = "Table X: Results for t-test comparing number of sites between medication and psychotherapy studies") %>% 
  kable_material(bootstrap_options = c("hover", "condensed", "bordered"), full_width = FALSE) %>% 
  row_spec(row = 0, background = "#EBEBEB") %>% 
  column_spec(column = 1, border_right = T)
...
Table X summarises the statistics of the estimated SMDs. <!--# Charlotte, we need to turn these into nice tables with proper columns, rounding etc. -->
...
df_mean_coefs_from_sim %>%
  kbl(caption = "Table X: Summary statistics of the estimated SMDs") %>% 
  kable_material(bootstrap_options = c("hover", "condensed", "bordered"), full_width = FALSE) %>% 
  row_spec(row = 0, background = "#EBEBEB") %>% 
  column_spec(column = 1, border_right = T)
