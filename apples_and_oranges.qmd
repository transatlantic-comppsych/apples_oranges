...
Psychotherapy trials featured a higher percentage of women when compared to medication trials. This subgroup difference in gender composition reached statistical significance (stats). Excluding sub-clinical studies yielded very similar results (stats). This difference in gender composition however did not reach statistical significance when only CBT, fluoxetine and escitalopram trials were included (stats). 
...
# Define function for baseline metaanalyses  ----------------------------------------------

# subgroup_df <- data.frame(psy_or_med = c(0, 1))
# 
# result <- list()
# name_vec <- character()
# 
# # Loop over rows in the subgroup_df
# for (i in 1:nrow(subgroup_df)) {
#   
#   # Call the metagen function for each row
#   result[[i]] <- metagen(
#     TE = percent_women,
#     seTE = percent_women_std_error,
#     studlab = new_study_id,
#     data = df_for_gender_metan[df_for_gender_metan$psy_or_med == subgroup_df$psy_or_med[i], ],
#     sm = "",
#     fixed = FALSE,
#     random = TRUE,
#     method.tau = "REML",
#     hakn = TRUE,
#     title = paste("Subgroup:", subgroup_df$psy_or_med[i])
#   )
#   
#   name_vec[i] = paste("Subgroup:", subgroup_df$psy_or_med[i])
# }
# 
# # Now result contains the metagen results for each subgroup
# # name_vec contains names for each subgroup
# 
# ## Function to extrac the key results for graphing and tabulating.
# 
...
# }
# 
# extract_stats(result[[1]])
# extract_stats(result[[2]])
# 
...
#                            title = "percentage women across studies")
# 
# subgroup_metan <-update.meta(met_perc_women_overall  , 
#                               subgroup = df_for_gender_metan$psy_or_med, subgroup.name = "treatment modality",
#                               tau.common = FALSE)

# Use the summary function with the 'byvar' argument
summary_met_perc_women <- summary(met_perc_women, byvar = "psy_or_med")

# Extract specific statistics for each subgroup
subgroup_names <- summary_met_perc_women$subgroup.name
TE_random <- summary_met_perc_women$TE.random
seTE_random <- summary_met_perc_women$seTE.random
lower_random <- summary_met_perc_women$lower.random
upper_random <- summary_met_perc_women$upper.random
pval <- summary_met_perc_women$pval.random
tau2 <- summary_met_perc_women$tau2
lower_tau2 <- summary_met_perc_women$lower.tau2
upper_tau2 <- summary_met_perc_women$upper.tau2

# Create a data frame to store the results
result_df <- data.frame(
  subgroup = subgroup_names,
  TE_random = TE_random,
  seTE_random = seTE_random,
  lower_random = lower_random,
  upper_random = upper_random,
  pval = pval,
  tau2 = tau2,
  lower_tau2 = lower_tau2,
...
Description of results X + table.
