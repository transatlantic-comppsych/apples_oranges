...
$$SMD_{change} = \frac{Mean_{t_{2}} - Mean_{t_{1}}}{\frac{SD_{t{2}} + SD_{t{1}}}2}\ $$ {#eq-4}
...
$$ SE_{change} = \sqrt{\frac{r_{t_{1}t_{2}}}{n} + \frac{SMD^2}{2n}}$$ {#eq-5}
...
# Charlotte to add in instrument end point values (mins / maxs)
# We will use these to transform the scale into % as follows: (score - min of the scale)/(max-min)
...
df_appl_v_orange <- df_appl_v_orange %>% 
  ...
  mutate(perc_baseline_sd_active = baseline_sd_active / (scale_max - scale_min)) %>%  # this #is the best way to estimate the SD of the linear transform of the percentage 
...
active_df <- df_with_distinct_instruments %>% 
  dplyr:: select(new_study_id, active_type, descr_active, psy_or_med, baseline_n_active, cohens_d_active,
                 ...
                 perc_baseline_mean_active, perc_baseline_sd_active, perc_post_mean_active, perc_post_sd_active, percent_women, percent_women_std_error, mean_age, sd_age) %>% 
...
control_df <- df_with_distinct_instruments %>% 
  dplyr:: select(new_study_id, control_type, descr_control, psy_or_med, baseline_n_control, cohens_d_control,
                 ...
                 perc_baseline_mean_control, perc_baseline_sd_control, perc_post_mean_control, perc_post_sd_control, percent_women, percent_women_std_error, mean_age, sd_age) %>% 
...
# I'm going to remove the March / TADS psy control row. It is currently filled with NAs which implies missing data, however it is actually the case that this condition doesn't exist. 
...
df_long_for_metan <- df_long_for_metan %>% 
  mutate (se_age = sd_age / sqrt(baseline_n) )
...
# use this dataframe to perform all operations below in quarto
...
Second, we restricted the metaregression to studies using CBT and the FDA-approved antidepressants fluoxetine and escitalopram.

Third, we restricted the metaregression to studies that *did not* use waitlist as their control.

Fourth, we tested whether our results could be accounted for by differential regression to the mean. In an initial step, we tested in metaregression as above for differences in baseline means. For this we used a min-max normalisation to turn each study arm mean score into a percentage using the following formalism:

$$
\text{outcome}_{percent} = \frac{\bar{X} - \text{scale}_{min}}{\text{scale}_{max} - \text{scale}_{min}}
$$ {#eq-14}

where, $$\bar{X}$$ is the mean score of each study arm on the primary questionnaire, and ${scale}_{min}$ and ${scale}_{max}$ are the minimum and maximum possible value of the scale in question, and for which the the standard deviation is calculated thus:

$$
\text{SD}_{Xpercent} = \frac{\text{SD}_{X}}{\text{scale}_{max} - \text{scale}_{min}}
$$ {#eq-15}

where $\text{SD}_{X}$ is the original standard deviation of the mean for X. Applying equations (12) and (13) on the baseline and outcome means and standard deviations of each study allowed us to then calculate SMDs and standard errors using equations (4) and (5)
...
# 
# cbt_study_ids <-
#   df_appl_v_orange[df_appl_v_orange$active_type == "cbt",]$new_study_id
# 
# cbt_studies_baseline_metareg <- all_studies_baseline_metareg %>%
#   filter(new_study_id %in% cbt_study_ids) %>% 
#   filter(!(arm_effect_size == "cohens_d_active" & !(type == "cbt")))
# 
# 
# #turn sds to sec
# cbt_studies_baseline_metareg$baseline_stand_error <- cbt_studies_baseline_metareg$perc_baseline_sd/
#                                         sqrt(cbt_studies_baseline_metareg$baseline_n)
# 
# 
# model_1 <- as.formula(~ four_level_var)
# condition <-  levels(list_df_simulated[[1]]$four_level_var) # this is the four level var 
#                                                               #throughout
# # also change this for the function to work
# cbt_studies_baseline_metareg$baseline_mean <- cbt_studies_baseline_metareg$perc_baseline_mean
# results_cbt_studies_baseline_metareg <- run_metareg_and_extract(cbt_studies_baseline_metareg,  
#                                                          model_1, condition)
...
df_for_gender_metan <- list_df_simulated[[1]] %>% 
...
met_perc_women <- metagen(TE = percent_women,
                           ...
                           subgroup = four_level_var,
                           tau.common = FALSE)
met_perc_women

# Now start looking at age variable

df_for_age_metan <- list_df_simulated[[1]] %>% 
  filter(!is.na(mean_age))

met_age <- metagen(TE = mean_age,
                           seTE = sd_age / sqrt(baseline_n),
                           studlab = new_study_id,
...
# ### get CDRS clin studies
#  clin_cdrs_studies_baseline_metareg <- cdrs_studies_for_baseline_metareg %>% 
#    filter(new_study_id %in% clin_study_ids)
#  
# 
# model_1 <- as.formula(~ four_level_var)
...
#                                                               #throughout
# 
# results_clin_cdrs_metareg_baseline <- run_metareg_and_extract(clin_cdrs_studies_baseline_metareg,  
#                                                          model_1, condition)
# 
# # Just realised this is identical, as all the studies that use the CDRS have clinical samples. 
# 
# ### get HAM-D clin studies
#  clin_hamd_studies_for_baseline_metareg <- hamd_studies_for_baseline_metareg %>% 
#    filter(new_study_id %in% clin_study_ids)
# 
# 
# model_1 <- as.formula(~ four_level_var)
# condition <-  levels(list_df_simulated[[1]]$four_level_var) # this is the four level var 
#                                                               #throughout
# results_clin_hamd_studies_for_baseline_metareg <- run_metareg_and_extract(clin_hamd_studies_for_baseline_metareg, 
#                                                          model_1, condition)
# 
# results_clin_hamd_studies_for_baseline_metareg$condition <-  levels(list_df_simulated[[1]]$four_level_var)
# 
# 
...
# model_adjust_cohens_d <-  lm(df_long_for_metan_adj_cohens_d$unadjusted_cohens_d ~ df_long_for_metan_adj_cohens_d$baseline_mean, na.action=na.exclude)
# 
...
We have also run models to take into accountregression to the mean within CDRS and HAMD.

```{# {r, warning=FALSE, message = FALSE, echo=FALSE}
# ####code to extract baseline-adjusted post means for CDRS and HAMD 
# 
#  
# 
# de_meaned_baselilne_mean_cdrs <- (list_cdrs_study[[1]]$baseline_mean - mean(list_cdrs_study[[1]]$baseline_mean, na.rm = TRUE)) 
# 
#                                     
# 
#  
...
#                                  
# 
# lm_for_cdrs_smds <- lm(list_cdrs_study[[1]]$post_mean ~ de_meaned_baselilne_mean_cdrs +list_cdrs_study[[1]]$four_level_var,
# 
#  
# 
...
#  
# 
# #summary(lm_for_cdrs_smds)
# 
#  
# 
# coefs_lm_for_cdrs_smds  <- broom:: tidy(lm_for_cdrs_smds)
# 
#  
# 
# condition <- coefs_lm_for_cdrs_smds$term
# 
# cdrs_smds <-c(coefs_lm_for_cdrs_smds$estimate[1], coefs_lm_for_cdrs_smds$estimate[-1] + coefs_lm_for_cdrs_smds$estimate[1])
# 
# cdrs_upper_ci <- cdrs_smds + coefs_lm_for_cdrs_smds$std.error*1.96
# 
# cdrs_lower_ci <- cdrs_smds - coefs_lm_for_cdrs_smds$std.error*1.96
# 
#  
# 
# cdrs_smd_with_cis <- data.frame (
# 
#   condition = levels(list_hamd_study$four_level_var),
# 
#   cdrs_smds = cdrs_smds[-2],
# 
#   cdrs_upper_ci = cdrs_upper_ci[-2],
# 
#   cdrs_lower_ci = cdrs_lower_ci[-2]
# 
# )
# 
#  
...
#  
# 
# # do the same for hamd
# 
# de_meaned_baselilne_mean_hamd <- (list_hamd_study$baseline_mean - mean(list_hamd_study$baseline_mean, na.rm = TRUE)) 
# 
#  
# 
#  
...
#  
# 
# lm_for_hamd_smds <- lm(list_hamd_study$post_mean ~ de_meaned_baselilne_mean_hamd +list_hamd_study$four_level_var,
# 
#                       
# 
#                        na.action=na.exclude,
...
#  
# 
# #summary(lm_for_hamd_smds)
# 
#  
# 
# coefs_lm_for_hamd_smds  <- broom:: tidy(lm_for_hamd_smds)
# 
#  
# 
#  
# 
# condition <- coefs_lm_for_hamd_smds$term
# 
# hamd_smds <-c(coefs_lm_for_hamd_smds$estimate[1], coefs_lm_for_hamd_smds$estimate[-1] + coefs_lm_for_hamd_smds$estimate[1])
# 
# hamd_upper_ci <- hamd_smds + coefs_lm_for_hamd_smds$std.error*1.96
# 
# hamd_lower_ci <- hamd_smds - coefs_lm_for_hamd_smds$std.error*1.96
# 
#  
# 
# hamd_smd_with_cis <- data.frame (
# 
# condition = levels(list_hamd_study$four_level_var),
# 
# hamd_smds = hamd_smds[-2],
# 
# hamd_upper_ci = hamd_upper_ci[-2],
# 
# hamd_lower_ci = hamd_lower_ci[-2]
# 
# )
# 
#  
