...
###########3. Function to extract parameters (coefs, ses, CI) for each level of the dummy #variable
...
############4. Function to get the means out of the extracted parameters
...
df_long_for_metan <- read.csv("df_long_for_metan") 
...
# extract coefs from the  list
list_dummy_var_means <-  lapply(list_model_1_meta_reg, extract_coefficients_func)

# calculate mean coefs and ses
df_mean_coefs_from_sim <- calculate_mean_coefs_ses(means_cbt_fluox_esc_study, conditions)
...
# first grab ids for those studies
...
# extract coefs from the list for cbt fluox escit
means_cbt_fluox_esc_study <-  lapply(list_model_1_cbt_fluox_esc_study, extract_coefficients_func)

# calculate mean coefs and ses
conditions <- unique(list_df_simulated[[1]]$four_level_var) # for the conditino argument
coef_and_se_means_cbt_fluox_esc_study <- calculate_mean_coefs_ses(means_cbt_fluox_esc_study, conditions)
...
no_wl_ids <-
  df_appl_v_orange[df_appl_v_orange$control_type != "wl",]$new_study_id
...
# apply the function to the list of cbt, fluox, esc
...
conditions <- unique(list_df_simulated[[1]]$four_level_var) # for the conditino argument
coef_and_se_means_no_wl <- calculate_mean_coefs_ses(means_no_wl, conditions)
coef_and_se_means_no_wl <- data.frame(cbind(condition,coef_and_se_means_no_wl))


 #add the ns of studies FIX THIS
...
df_mean_coefs_from_sim$text_label[i] <-  paste(
   ...
   round(df_mean_coefs_from_sim$coef_means[i], 2),
   "[" ,
...
plot_means <- ggplot(df_mean_coefs_from_sim, aes(x = coef_means, y = 1:nrow(df_mean_coefs_from_sim),
...
We then examined whether restricting studies to the trials (US-) licensed medication fluoxetine and escitalopram and the most widely used and evidence-based psychological treatment, namely CBT would change results. As can be seen, the coefficient of the difference between placebo and psychotherapy control condition is very strong as judged by the z-value of the difference (8.8), and comaprable to the overall results.

```{r, warning=FALSE, message = FALSE, echo=FALSE}
# filter dataset to include only those ids with cbt fluox or escitalopram

# grab those ids
cbt_fluox_esc_study_ids <-
  df_appl_v_orange[df_appl_v_orange$active_type == "cbt" | df_appl_v_orange$active_type == "Fluoxetine" | df_appl_v_orange$active_type == "Escitalopram",]$new_study_id

# subset all dataframes in the list
list_cbt_fluox_esc_study <- list()
...
# apply the function to the list of cbt, fluox, esc
list_model_1_meta_reg_cbt_fluox_esc <- run_metaregression(list_cbt_fluox_esc_study, model_1)


# now use the function to extract the coefficients

list_model_1_meta_reg_cbt_fluox_esc[[1]]
```
