...
NOTE: CHARLOTTE Please create a table here with the results from the count studies or study count variables that you checked in the code chunk above for each A,B and C. 
The table for A should be added here and accompany the description below. 
THe other two should go below as supplementary.
As indicated in the methods we should have one overall table with all studies and all their characteristics ala Cipriani.
...
Figure 1 graphs the results with the means of each level alongside their confidence intervals. Table 1 shows the results containing the means and standard errors for each level, derived as the average of 1000 metaregressions. 
