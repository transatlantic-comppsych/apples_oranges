...
df_appl_v_orange <- df_appl_v_orange %>% 
  ...
  mutate(perc_baseline_sd_active = (baseline_sd_active - scale_min) / (scale_max - scale_min)) %>% 
  mutate(perc_baseline_mean_control = (baseline_mean_control - scale_min) / (scale_max - scale_min)) %>% 
  mutate(perc_baseline_sd_control = (baseline_sd_control - scale_min) / (scale_max - scale_min)) %>% 
  mutate(perc_post_mean_active = (post_mean_active - scale_min) / (scale_max - scale_min)) %>% 
  mutate(perc_post_sd_active = (post_sd_active - scale_min) / (scale_max - scale_min)) %>% 
  mutate(perc_post_mean_control = (post_mean_control - scale_min) / (scale_max - scale_min)) %>% 
  mutate(perc_post_sd_control = (post_sd_control - scale_min) / (scale_max - scale_min)) 
...
active_df <- df_with_distinct_instruments %>% 
  dplyr:: select(new_study_id, active_type, descr_active, psy_or_med, baseline_n_active, cohens_d_active,
                 ...
                 perc_baseline_mean_active, perc_baseline_sd_active, perc_post_mean_active, perc_post_sd_active) %>% 
...
control_df <- df_with_distinct_instruments %>% 
  dplyr:: select(new_study_id, control_type, descr_control, psy_or_med, baseline_n_control, cohens_d_control,
                 ...
                 perc_baseline_mean_control, perc_baseline_sd_control, perc_post_mean_control, perc_post_sd_control) %>% 
...
#this will be important later in checking for multi arm psy studies 
check_active_dups <- df_long_for_metan %>%
  filter(new_study_id %in% cbt_fluox_esc_study_ids) %>% 
  filter (psy_or_med == 1) %>% 
  group_by(new_study_id) %>%
  filter(n() >= 3)
write.csv(check_active_dups, "check_active_dups.csv")
...
Fourth, we tested whether our results could be accounted for by differential regression to the mean. In an initial step, we tested in metaregression as above for differences in baseline means for those medication and psychotherapy studies that shared the same instruments. We then accounted for the effect that possible differing baseline values across all studies studies' outcomes by regressing the SMDs of studies on their respective baseline values of the instrument with which the SMD was calculated. For example, in a study that used the CDRS, this was used to calculate the SMD. This SMD was then used as the outcome and the baseline value of the CDRS, termed *BaselineScore* below, in that study was used as the independent variable as follows: $$
SMD = \beta_0 + \beta_1 BaselineScore + \epsilon
$$ {#eq-12}

This allows us to estimate the effect of the baseline score on the SMD by evaluating $\beta_1$. We can then re-arrange Equation 12 to extract the residuals, that is, the variability in the SMD that is not accounted for by the baseline score.

$$
\epsilon = SMD - (\beta_0 + \beta_1 BaselineScore) ) \
$$

where, $\epsilon$ is the adjusted SMD, for which we simulated standard errors and then used it the outcome in metaregression, as described above for the unadjusted SMDs.
...
view(list_cbt_fluox_esc_study[[1]])
...
# apply the metareg function to the list of cbt, fluox, esc
list_model_1_no_wl_or_cau <- run_metaregression(list_no_wl_or_cau , model_1)

# extract coefficients and model characteristics.
condition <- levels(list_df_simulated[[1]]$four_level_var)
aggregate_results_no_wl_or_cau <-  aggregate_model_results(list_model_1_no_wl_or_cau, condition)
aggregate_results_no_wl_or_cau <- cbind(aggregate_results_no_wl_or_cau, condition)
...
```{r, warning=FALSE, message = FALSE, echo=FALSE}
knitr:: kable(results_hamd_metareg_baseline[[2]], caption="Table 3 HAM-D baselilne scores")
```
