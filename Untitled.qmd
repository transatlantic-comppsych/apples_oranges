...
test_summary <- df_with_distinct_instruments %>% 
  group_by(psy_or_med) %>% 
  summarise(rr_active = mean(resp_rate_active, na.rm = TRUE),
            rr_control = mean(resp_rate_control, na.rm = TRUE),
            n = n())
...
aggregate_results_overall %>%
  kbl(caption = "Meta-regression results") %>%
  kable_material(lightable_options = c("hover"), full_width = FALSE) %>%
  row_spec(row = 0, background = "#EBEBEB") %>%
  column_spec(column = 1, border_right = T)
...
control_summary %>%
  kbl(align = "l", caption = "Comparing the intensity of the intervention between active and control arms of psychotherapy studies", escape = F) %>%
  kable_material(lightable_options = c("hover"), full_width = FALSE) %>%
  row_spec(row = 0, background = "#EBEBEB", extra_css = "border-bottom: 1px black;" ) %>% 
  column_spec(1, border_right = T) %>% 
...
control_summary_t_tests %>%
  kbl(align = "l", caption = "Results for t-tests comparing intervention intensity between active and control arms of psychotherapy trials") %>% 
  kable_material(lightable_options = c("hover"), full_width = FALSE) %>% 
  row_spec(row = 0, background = "#EBEBEB") %>% 
  column_spec(column = 1, border_right = T)
