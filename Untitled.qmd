...
flextable(df_overall_baseline_results) %>%
  set_caption (autonum = TRUE, "Sample characteristics at baseline across medication and psychotherapy studies") %>%
...
flextable(df_sites_summary_stats) %>%
  set_caption (autonum = TRUE, "Number of sites across medication and psychotherapy studies", style = "Table Caption") %>%
  bold(part = "header") %>%  
...
flextable(sites_t_test_stats) %>%
  set_caption (autonum = TRUE, "Results for t-test comparing number of sites between medication and psychotherapy studies", style = "Table Caption") %>%
  bold(part = "header") %>%  
  border_outer(border = small_border) %>%  
  hline(part = "header", border = big_border) %>% 
  set_table_properties(width = .75) %>% 
...
flextable(df_mean_coefs_from_sim) %>%
  set_caption (autonum = TRUE, "Summary statistics of the estimated SMDs") %>%
  bold(part = "header") %>%  
  border_outer(border = small_border) %>%  
  hline(part = "header", border = big_border) %>% 
  set_table_properties(width = .75) %>% 
...
flextable(aggregate_results_overall) %>%
  set_caption (autonum = TRUE, "Metaregression results") %>%
  bold(part = "header") %>%  
  border_outer(border = small_border) %>%  
  hline(part = "header", border = big_border) %>% 
  set_table_properties(width = .75) %>% 
...
as_grouped_data(control_summary, groups = "Column") %>% 
  ...
  set_caption (autonum = TRUE, "Comparing the intensity of the intervention between active and control arms of psychotherapy studies") %>%
  bold(part = "header") %>%
...
flextable(control_summary_t_tests) %>%
  set_caption (autonum = TRUE, "Results for t-tests comparing intervention intensity between active and control arms of psychotherapy trials", style = "Table Caption") %>%
  bold(part = "header") %>%  
  border_outer(border = small_border) %>%  
  hline(part = "header", border = big_border) %>% 
  set_table_properties(width = .75) %>% 
