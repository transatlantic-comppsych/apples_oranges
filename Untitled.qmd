...
When taking only those studies where the active arm was either CBT, fluoxetine or escitalopram, there was a significant difference between psychotherapy and medication studies. This difference did not reach statistical significance when excluding psychotherapy studies that recruited samples with sub-clinical depression or when excluding studies that used wait list as their control. Please refer to Table XXXXX for these results. <!-- Charlotte, please point to the Supplementary Table where these results live -->
...
As can be seen in Table 1, psychotherapy trials featured a significantly higher percentage of women when compared to medication trials. On average, samples were comprised of `r round(results_met_perc_women_overall$TE_random[results_met_perc_women_overall$subgroup == 1][[2]],1)`% (*SE* = `r round(results_met_perc_women_overall$seTE_random[results_met_perc_women_overall$subgroup == 1][[2]],1)` women across psychotherapy trials and `r round(results_met_perc_women_overall$TE_random[results_met_perc_women_overall$subgroup == 0][[2]],1)`% (*SE* = `r round(results_met_perc_women_overall$seTE_random[results_met_perc_women_overall$subgroup == 0][[2]],1)` women across medication trials. Excluding sub-clinical and waitlist control studies yielded very similar results. This difference in gender composition however did not reach statistical significance when only CBT, fluoxetine and escitalopram trials were included . <!-- Charlotte please refer to Supple Table -->
...
#df_overall_baseline_results <- options(knitr.kable.NA = "") # CHARLOTTE THIS IS PROBABLY OFFENDING
...
![Figure 1.](plot_means_all.pdf){alt="Figure 1."}

```{r, warning=FALSE, message = FALSE, echo=FALSE}
# df_mean_coefs_from_sim
df_mean_coefs_from_sim %>%
  kbl(align = "l", caption = " Summary statistics of the estimated SMDs") %>%
  kable_material(lightable_options = c("hover"), full_width = FALSE) %>%
  row_spec(row = 0, background = "#EBEBEB") %>%
  column_spec(column = 1, border_right = T)
...
aggregate_results_overall %>%
  kbl(caption = "Table 5: Meta-regression results") %>%
  kable_material(lightable_options = c("hover"), full_width = FALSE) %>%
  row_spec(row = 0, background = "#EBEBEB") %>%
  column_spec(column = 1, border_right = T)
