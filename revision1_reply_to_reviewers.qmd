...
subtitle: "Major Revision: Letter to Reviewers"
format: 
  docx:
    fig-width: 9
    fig-height: 7
...
#set source
load("my_environment.RData") #image that contains objects after main qmd has been run
```
...
This is to not unduly bias the results. We now present the analyses with all studies included in the Supplemental Materials (see @tbl-all-studies-percfemale-results).
...
$$
NNT_{Trial_1} = \frac{1}{0.05} = 20 
$$

whereas:

$$
NNT_{Trial_2} = \frac{1}{0.3} = 3.3 
$$
...
$$
Eff_{A} = A - A_{Control} = 90% - 60% = 30%
$$

and that of B as:

$$
Eff_{B} = B - B_{Control} = 40% - 10% = 30%
$$
...
png("mean_diff_min_max_scores.png")
...
coef_and_se_means_var2_study <- coef_and_se_means_var2_study %>% 
  mutate(
    condition = case_when(condition == "medication_control" ~ "Medication Control", condition == "medication_active" ~ "Medication Active", condition == "psychotherapy_control" ~ "Psychotherapy Control",
condition == "psychotherapy_active" ~ "Psychotherapy Active", TRUE ~ as.character(condition))) 
